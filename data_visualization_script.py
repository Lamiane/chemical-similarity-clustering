...
def data_as_image(with_diag, no_diag, title):
    ...
    try:
        ...
        plt.imshow(with_diag.astype('float'))
    ...
    try:
        ...
        plt.imshow(no_diag.astype('float'))
...
def slownik_dopasowan(bin_sim_, scale_sim_, bin_sim_no_identity_=None):
    # TODO: docs
        bi_sc_dict_ = {}
        sc_bi_dict_ = {}

        # TODO: to jest mega brzydkie, uwaga z tych f-cji korzysta ten kod i kod 30x30. Uladnic!
        if bin_sim_no_identity_ is None:
            itr = itertools.permutations(xrange(bin_sim_.shape[0]), 2)
        else:
            itr = zip(*bin_sim_no_identity_.nonzero())
        ...
        for i, j in itr:
            bi = bin_sim_[i, j]
            sc = scale_sim_[i, j]
            if bi not in bi_sc_dict_:
                bi_sc_dict_[bi] = []
            bi_sc_dict_[bi].append(sc)
            if sc not in sc_bi_dict_:
                sc_bi_dict_[sc] = []
            sc_bi_dict_[sc].append(bi)

        for key in bi_sc_dict_:
            bi_sc_dict_[key] = collections.Counter(bi_sc_dict_[key])

        for key in sc_bi_dict_:
            sc_bi_dict_[key] = collections.Counter(sc_bi_dict_[key])
        return bi_sc_dict_, sc_bi_dict_


def dopasowania(slownik_dopasowan_, title):
    """
    Plot image showing how many pairs that have x note in one scale have y note in another scale

    Parameters
    ----------
    slownik_dopasowan_: dict
    ...
    plt.suptitle(title)
    for en, key in enumerate(slownik_dopasowan_):
        counts = []
        labels = []
        for kkey in slownik_dopasowan_[key]:
            counts.append(slownik_dopasowan_[key][kkey])
            labels.append(kkey)
        plt.subplot(1, len(slownik_dopasowan_.keys()), en + 1)
        plt.title(key)
        plt.axis('equal')
...
if __name__ == '__main__':
    ...
    bi_sc_dict, sc_bi_dict = slownik_dopasowan(bin_sim, scale_sim, bin_sim_no_identity)
